<template name="yq-avatar">
	<view>
		<view class="button11" @click="fSelect">
			上传头像
		</view>
	...
	export default {
		...
		methods: {
			...
			fDrawImage(ini = false) {
				...
				if (this.bgImage) {
					ctxCanvas.drawImage(this.bgImage, -this.posWidth - this.useWidth / 2, -this.posHeight - this.useHeight / 2, this.windowWidth,
						this.windowHeight - tabHeight);
				} else {
					ctxCanvas.fillRect(-this.posWidth - this.useWidth / 2, -this.posHeight - this.useHeight / 2, this.windowWidth,
						this.windowHeight - tabHeight);
				}
